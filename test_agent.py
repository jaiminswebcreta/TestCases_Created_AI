...
def generate_test_cases(api_key, user_description, num_cases, categories, prefix, image_parts, ocr_text):
    ...
    final_prompt = MASTER_PROMPT.format(user_description=user_description, num_cases=num_cases, categories_to_include=', '.join(categories), case_id_prefix=prefix, ocr_text=ocr_text)
...
def generate_bug_summary(api_key, bug_df):
    genai.configure(api_key=api_key)
    bug_list_md = bug_df.to_markdown(index=False)
    summary_prompt_formatted = BUG_SUMMARY_PROMPT.format(bug_list_markdown=bug_list_md)
    model = genai.GenerativeModel(model_name=AI_MODEL_NAME)
    response = model.generate_content(summary_prompt_formatted)
    return response.text

# --- Initialize Cookie & Session State ---
...
with st.sidebar.expander("🔬 Test Generation Settings", expanded=True):
    ...
    test_categories = st.multiselect(
        "Select Test Categories to Generate",
        options=["Positive Scenarios", "Field-Level Validations", "Edge Cases", "UI/UX Scenarios", "Functional Logic", "System Behavior", "Accessibility"],
        default=["Positive Scenarios", "Field-Level Validations", "Edge Cases", "UI/UX Scenarios"]
    )
...
with st.sidebar.expander("📜 View Generation History (Last 5)"):
    if not st.session_state.history: st.info("Your generation history will appear here.")
        ...
        for i, record in enumerate(st.session_state.history):
            if st.button(f"Restore Generation {i+1}", key=f"hist_btn_{i}", use_container_width=True):
...
st.markdown("Your one-stop tool to generate, edit, and export comprehensive test suites from UI screenshots.")

main_cols = st.columns(2)
with main_cols[0]:
    st.subheader("Your Input")
    uploaded_files = st.file_uploader("1. Upload UI Screenshot(s) (supports multi-screen flows)", type=["png", "jpg", "jpeg"], accept_multiple_files=True, key=f"uploader_{st.session_state.run_id}")
    if uploaded_files:
        st.write("Uploaded Image Previews:"); cols = st.columns(len(uploaded_files))
        for i, uploaded_file in enumerate(uploaded_files):
            with cols[i]: st.image(uploaded_file, use_container_width=True, caption=f"Screen {i+1}")
    user_description = st.text_area("2. Add a short description for context", height=100, placeholder="Example: This is the user login page...", key=f"description_{st.session_state.run_id}")
    with st.expander("💡 Need help with the description? (Prompt Tips)"):
        st.info("""**Good descriptions lead to better test cases!**\n- **Weak:** "login test"\n- **Strong:** "This is the main login screen for our B2B SaaS platform. It requires a corporate email and a password. There is a 'Forgot Password' link but no 'Sign Up' option. After 3 failed attempts, the account should be locked." """)
    st.markdown("3. Generate your test suite!")
...
with main_cols[1]:
    ...
    if st.session_state.last_run_inputs:
        if st.button("🔁 Regenerate Last Response"):
            st.session_state.last_run_inputs['regenerate'] = True
            st.rerun()
    if st.session_state.df_test_cases.empty:
        st.info("The generated Module Description and Test Cases will appear here after generation.")
    else:
        ...
        with tab1:
            ...
            aggrid_response = AgGrid(st.session_state.df_test_cases, gridOptions=gb.build(), update_mode=GridUpdateMode.MODEL_CHANGED, data_return_mode=DataReturnMode.AS_INPUT, allow_unsafe_jscode=True, height=500, fit_columns_on_grid_load=True, theme='streamlit')
            edited_df = pd.DataFrame(aggrid_response['data'])
            st.session_state.df_test_cases = edited_df
            st.divider()
            ...
            with export_col1: st_copy_button(get_spreadsheet_ready_tsv(edited_df), "📋 Copy for Excel/Sheets")
            with export_col2: st.download_button("📄 Download as CSV", edited_df.to_csv(index=False).encode('utf-8'), "test_suite.csv", "text/csv", use_container_width=True)
            with export_col3: st.download_button("📑 Download as PDF", export_df_to_pdf(edited_df), "test_suite.pdf", "application/pdf", use_container_width=True)
        with tab2:
...
def trigger_generation(run_inputs):
    with st.spinner("🧠 AI is analyzing and writing your test suite... This may take a moment."):
        try:
            ocr_texts, image_parts = [], []
            for uploaded_file_data in run_inputs['files']:
                ocr_texts.append(extract_text_from_image(uploaded_file_data))
                image_parts.append(Image.open(io.BytesIO(uploaded_file_data)))
            ...
            if not df.empty:
                ...
                st.session_state.df_test_cases = pd.DataFrame()
                st.warning("The AI returned a response, but no valid test case table could be parsed.")
...
# --- Footer ---
st.divider()
st.markdown(f"<div style='text-align: center; color: grey;'>Built with ❤️ by Jaimin Sharma | AI Model: {AI_MODEL_NAME}</div>", unsafe_allow_html=True)